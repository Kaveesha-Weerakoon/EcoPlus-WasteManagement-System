  ...
  class Admin extends Controller {
    ...
    public function addadmins2(){
      if(isset($_SESSION['superadmin_id']) ){
        ...
        if(empty($data['like'])){
          $data['like_err'] = 'Please enter yes or no';
        }
        elseif (strlen($data['name']) > 255) {
          $data['like_err'] = 'like is too long ';
        }

      
      

        if(empty($data['email_err']) && empty($data['name_err']) && empty($data['password_err']) && empty($data['confirm_password_err']) && empty($data['contact_no_err']) && empty($data['nic_err']) && empty($data['address_err']) && empty($data['dob_err'])  && empty($data['like_err'])){
          // Validated
          $data['password'] = password_hash($data['password'], PASSWORD_DEFAULT);
...
/*hashpublic function login($email, $password){
